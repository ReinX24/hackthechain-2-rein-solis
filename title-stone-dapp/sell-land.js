  ...
  async function loadListings() {
    try {
      ...
      listings.forEach((land) => {
        ...
        const buyButtonHTML = land.isAvailable
          ? `<button 
          ...
          : `<span class="text-gray-400 italic">Sold</span>`;
        ...
        row.innerHTML = `
          ...
          <td class="p-2">${ethers.formatEther(land.price)} VIC</td>
      ...
      document.querySelectorAll(".buy-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          try {
            ...
            await loadListings();
            window.location.reload(); // Full refresh to update table
          } catch (err) {
            console.error(err);
