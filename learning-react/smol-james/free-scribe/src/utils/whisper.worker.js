...
const load_model_callback = async (data) => {
  const { status } = data;
  if (status === "progess") {
...
const sendLoadingMessage = (status) => {
  self.postMessage({
...
const sendDownloadingMessage = async (file, progess, loaded, total) => {
  self.postMessage({
...
function createPartialResultMessage(result) {
  self.postMessage({ type: MessageTypes.RESULT_PARTIAL, result });
}
