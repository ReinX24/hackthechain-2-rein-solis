  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      ...
      <div className="flex flex-col bg-slate-950 rounded gap-2">
        {exercise.description.split("___").map((val, valId) => {
          return (
            <div key={valId} className="text-sm">
              {val}
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info, keyInfo) => {
          return (
            <div
              key={keyInfo}
              className="flex flex-col p-2 rounded border-[1.5px] border-solid borde-slate-900 w-full"
            >
